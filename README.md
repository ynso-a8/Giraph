...
## 사용 기술 스택
- Core Framework: Next.js (v16.2.6 App Router) & React (v19)
  - 유연하고 빠른 라우팅과 정적 HTML 파일 내보내기(output: 'export')를 통한 모바일 패키징을 동시 지원합니다.
- Language: TypeScript (v5.x)
  - 정적 타입 지정을 통해 코드의 무결성과 안정성을 보장합니다.
- Styling & Design: Tailwind CSS (v4.0.0)
  - 유틸리티 퍼스트 CSS와 감성 컬러 테마 변경 기능(라벤더, 선셋, 초록, 밀크티)을 구현했습니다.
- Database & Storage: Supabase & LocalStorage (이중화 설계)
  - 온라인: PostgreSQL 기반의 Supabase 클라우드 데이터베이스에 실시간 동기화합니다.
  - 오프라인/로컬: Supabase 연결 해제 시 브라우저 내장 LocalStorage로 자동 복구(Fallback)되어 중단 없이 동작합니다.
- Data Visualization: Recharts (v3.8.1)
  - 감정 수치 변화를 선형 차트(기린 마커) 및 막대 차트(목이 길어지는 기린 디자인)로 커스텀 시각화합니다.
- Mobile Shell (모바일 래퍼): Capacitor (v6.0.0)
  - 정적 빌드된 웹 코드를 안드로이드 네이티브 앱(APK)으로 패키징 및 구동합니다.
