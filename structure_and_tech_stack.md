# 🦒 기래프 (Giraph) 기술 스택 & 폴더 구조 정의서

**기래프(Giraph - 내가 그린 기분 그래프)**는 사용자의 매일 기분을 기록(기-log)하고 시각화(기래프)하며, 자가 진단을 통한 AI 기반 마인드케어 분석 리포트를 제공하는 하이브리드 모바일 및 웹 서비스입니다. 

본 문서는 프로젝트의 전체 기술 스택 구성, 디렉토리 구조 및 핵심 기능 아키텍처를 명확히 파악할 수 있도록 정리한 아키텍처 문서입니다.

---

## 🛠️ 기술 스택 (Technology Stack)

프로젝트 전반에 사용된 주요 프레임워크, 라이브러리 및 도구 명세입니다.

| 분류 | 기술 요소를 구성하는 항목 | 버전 / 세부 사항 | 설명 |
| :--- | :--- | :--- | :--- |
| **Core** | **Next.js** | `v16.2.6` (App Router) | React 기반 프레임워크로, 정적 내보내기(output: 'export')를 통한 하이브리드 모바일 리소스 연동 지원 |
...
| **AI Service**| **Gemini API** | `gemini-2.5-flash` | 경량/고속 LLM을 연동하여 AI 자가진단 처방 리포트 생성, 1:1 고민 상담 챗봇 및 미래 위로 타임캡슐 편지 생성 |
...
프로젝트 루트 디렉토리 및 주요 파일 목록입니다. (모바일 래퍼 프로젝트 `Giraph/` 포함)

```text
mood-tracker (Root)
├── .git/                      # Git 버전 관리 정보
├── .gitignore                 # Git 제외 대상 목록 설정
├── .env.local                 # Supabase 및 Gemini API Key 연동 환경 변수 설정 파일
...
│   │   ├── capsule/
│   │   │   └── page.tsx       # [메뉴] 타임캡슐 페이지 (과거 감정 회상, 지정 조건부 편지 예약 및 마음 건강 지수 제공)
│   │   └── help/
...
│       ├── aiService.ts       # Gemini 2.5 Flash API 연동 및 비동기 심층 진단/상담 챗봇 비즈니스 로직
│       ├── moodService.ts     # 기분 기록 데이터 CRUD 및 자가진단 분석 내역 관리 (Supabase & LocalStorage Fallback)
│       ├── sampleLogs.ts      # 56개 샘플 기분 기록 및 2개 상세 처방 리포트 더미 데이터셋
...
* **`globals.css`**: Tailwind v4 문법을 기본으로 하면서, 각 감성 테마에 매핑된 CSS 변수 토큰 정의와 애니메이션(`giraffe`, `fade-in` 등)을 선언했습니다.
...
### 3. `src/lib` (핵심 비즈니스 및 AI 서비스 레이어)
* **`aiService.ts`**: `gemini-2.5-flash` API 엔드포인트를 호출하는 fetch 기반 서비스 유틸리티입니다. 모바일 Capacitor WebView 환경에서의 통신 제약을 최소화하기 위해 외부 라이브러리 의존성 없이 표준 `fetch` API를 사용하여 비동기로 구현되었으며, API Key 미설정 시 로컬 키워드 매핑 알고리즘으로 안전하게 Fallback 하도록 돕는 `isGeminiConfigured()` 함수를 탑재하고 있습니다.
* **`moodService.ts`**: 데이터 저장 및 조회 로직을 제어합니다. Supabase 서버 연결 상태를 감지하여 연결 불가 시 LocalStorage로 투명하게 전환하는 Fallback 로직이 내장되어 있습니다.

### 4. 🤖 AI 마음케어 & 테스트 데이터 제어 (AI Mindcare & Test Data Control)
사용자 및 심사자가 앱의 핵심 기능을 신속하고 유실 없이 테스트할 수 있도록 특화된 데이터 제어 시스템이 설계되어 있습니다.
* **병합형 테스트 데이터 제어 (Non-destructive Merge & Delete)**:
  - 기-log 탭(홈) 및 자가진단/리포트 탭(분석/일지) 상단에 배치된 '테스트 데이터 생성/삭제' 버튼 동작 시, 사용자가 직접 기록한 데이터를 덮어써서 유실시키는 방식이 아닌 **기존 데이터에 머지(Merge)**하는 방식을 채택했습니다.
  - 테스트 데이터 삭제 시에는 `sampleLogs.ts`에 정의된 더미 데이터의 고유 ID 목록만을 추적해 선택적으로 필터링 및 제거하므로, **테스트 데이터를 넣고 활성화해둔 채 사용자가 새로 작성한 실제 기록들도 완벽히 유지**됩니다.
* **실시간 멀티 탭/페이지 동기화**:
  - 로컬 스토리지에 데이터가 추가/삭제된 즉시 윈도우에 `storage` 이벤트를 전파하여, 분석 탭에서 테스트 데이터를 켜거나 끄면 리포트 일지 화면이 새로고침 없이 즉각적으로 반영됩니다.

### 5. `Giraph/` (Capacitor 네이티브 패키징)
* Next.js의 정적 파일 내보내기(`next export` 결과물로 빌드된 `out/` 폴더)를 활용해 웹 브라우저 코드를 그대로 모바일 앱 환경으로 래핑합니다. `capacitor.config.ts`에서 빌드 출력 디렉토리를 `../out`으로 타겟팅하여 안드로이드 디렉토리인 `android/` 환경과 즉시 동기화(Sync)할 수 있습니다. 안드로이드 스튜디오를 통해 실제 하드웨어 스마트폰 기기에 바로 디버그 빌드 및 배포할 수 있습니다.

### 6. `supabase/` (데이터 모델 설계)
...
---
*본 아키텍처 정의서는 기래프 하이브리드 앱 서비스의 높은 유지 보수성과 독립적인 오프라인 구동 아키텍처, 그리고 프리미엄 마인드케어 비즈니스 아키텍처를 정의합니다.*
